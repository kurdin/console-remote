  ...
  root[name] = (function() {
    ...
    function onConnect() {
      ...
      if (socket) {
        ...
        if (cli) a = 'Sent';
        console.log(a+" to http://" + chost + "/"+channel);
      }
      sendCached(cache);
      if (cli) socket.disconnect();
    }
    ...
    api.connect = function(redux) {
      if (io) {
        socket = io.connect("http://" + chost + ":" + cport, {
          'resource': ctransport
        });
        socket.on('connect', onConnect);
        socket.on('error', function(e){ console.log('console.re [error] '+e);});
      }
      else if (!redux) {
        api.connect(true);
      }
    ...
    api.trace = function() {
      ...
      try { throw new Error(''); } catch(err) {
        es = err.stack.replace(/^\s+|\s+$/gm, '').split("\n").slice(2);
