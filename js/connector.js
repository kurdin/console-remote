/*! Socket.IO.min.js build:1.2.1, production. Copyright(c) 2014 LearnBoost <dev@learnboost.com> MIT Licensed */
...
// Console.Re Client Script ver: 0.4.8
...
(function(root, console) {
	...
	function connect(host, port, _channel) {
		chost = host || 'console.re';
		cport = '';
		channel = _channel;

		var cprotocol = port === '443' ? 'https://' : 'http://';
	...
	root[name] = (function() {
		...
		api.connect = function(redux) {
			if (root.io) {
				socket = root.io.connect(chost + (typeof cport !== 'undefined' ? ':' + cport : ''));
